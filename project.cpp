...
public:
    void moveRecursive(const fs::path& source, const fs::path& destination) {
        std::vector<std::thread> threads;

        for (const auto& entry : fs::recursive_directory_iterator(source)) {
            threads.emplace_back([&](const fs::directory_entry& e) {
                fs::copy(e, destination / e.path().filename(), fs::copy_options::overwrite_existing);
                fs::remove_all(e);
            }, entry);
        }
...
public:
    void execute(const std::vector<std::string>& args) override {
        ...
        if (recursive) {
            listDirectoryRecursive(fs::current_path(), long_format, reverse_order);
        } else {
            listDirectory(fs::current_path(), long_format, reverse_order);
        }
    }

private:
    void listDirectory(const fs::path& path, bool long_format, bool reverse_order) const {
    ...
    void listDirectoryRecursive(const fs::path& path, bool long_format, bool reverse_order) const {
        std::vector<std::thread> threads;

        for (const auto& entry : fs::directory_iterator(path)) {
            if (fs::is_directory(entry)) {
                threads.emplace_back([&](const fs::directory_entry& e) {
                    listDirectory(e, long_format, reverse_order);
                }, entry);
            } else {
                printEntry(entry, long_format);
            }
